<%-- 
    Document   : odaFiyatlariGuncelle
    Created on : 31.May.2022, 20:31:55
...
<div class="genelAlan">
    ...
    <table class="tablo" border="1">
        <tr>
            <td class="anatd">Oda Numarası</td>
        ...
        <%
            ...
            ResultSet rs = st.executeQuery("select * from otelyonetim.odafiyat");
            try {
                int i = 0;
                while (rs.next()) {
        %>
        <tr>
        <form action="odaFiyatlariGuncelleDatabase.jsp">
            <td><%=rs.getString("oda")%></td>
            <td><%=rs.getString("fiyat")%></td>
            <td><input type="input" name="yeniFiyat" required></td>
                <input type="hidden" id="<%=rs.getString("oda")%>" name="oda" value="<%=rs.getString("oda")%>">
